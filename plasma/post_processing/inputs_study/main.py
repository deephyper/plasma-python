...
def plot_signal_group(ax, signals, sig_nums, lim):
...
def plot_shot(shot, idx):
    shot.restore(conf['paths']['processed_prepath'])
    disr = " - disr" if shot.is_disruptive else ""
    ...
    ax[0].set_title(f"test - shot_{idx}{disr}")
    ...
    plt.tight_layout()
    plt.savefig(f"plots/shot_{idx}.png")
    plt.clf()

def plot_shot_separated(shot, idx):
    shot.restore(conf['paths']['processed_prepath'])
    normalizer.apply(shot)
    signals = list(shot.signals_dict.values())
    plt.figure()
    ...
    plt.tight_layout()
    plt.savefig(f"plots/shot_{idx}_profiles.png")
    plt.clf()
    ...
    plt.xlabel("timestep (in ms.)")
    plt.tight_layout()
    plt.savefig(f"plots/shot_{idx}_scalars.png")
    plt.clf()
