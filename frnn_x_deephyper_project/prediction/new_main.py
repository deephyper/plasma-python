...
from utils.algorithms import apply_calibration_model, apply_balanced_calibration_model, apply_calibration_models, apply_balanced_calibration_models, apply_caruana, old_apply_calibration_ensemble, apply_calibration_ensemble, loss_balanced_crossentropy, loss_auc
...
if __name__ == "__main__":
    ...
    if method != "new_method":
        ...
        y_pred_best_model = apply_balanced_calibration_model(y_pred_best_model, y_true, dataset, subset, criteria, models[0], group=group)
        t = time.time()
        y_pred_models, weights, ensemble = old_apply_calibration_ensemble(y_pred_models, y_true, dataset, subset, criteria, group, keep_models_thresh=keep_model_thresh)
        T += time.time() - t
        ...
        y_pred_best_model = apply_balanced_calibration_model(y_pred_best_model, y_true, dataset, subset, criteria, models[0], group=group)
        t = time.time()
        y_pred_models, sample_weights = apply_calibration_ensemble(y_pred_models, y_true, dataset, subset, criteria, group)
        T += time.time() - t
    t = time.time()
    y_pred_models_selection = []
    for y_pred in y_pred_models:
        y_pred_models_selection.append(y_pred[ensemble, ...])
    y_pred_ensemble = average_prediction(y_pred_models_selection, weights=weights, sample_weights=sample_weights)
    T += time.time() - t
    ...
    #     plot_pred_uq_shot_single_model(y_pred_baseline, y_true, u_base, subset, f"{subset}/baseline", i, suffix="pdf")
    #     # plot_pred_uq_shot_single_model(y_pred_best_model, y_true, u_best, subset, f"{subset}/best_model", i, suffix="png")
    #     # plot_pred_uq_shot_single_model(y_pred_best_model, y_true, u_best, subset, f"{subset}/best_model", i, suffix="pdf")
    ...
    #         # plot_pred_uq_shot(y_pred_ensemble, y_true, alea, epis, tota, subset, f"{subset}/{group}/{method}/{calibrator}", i, suffix="pdf")
    ...
    #         # plot_pred_uq_shot(y_pred_ensemble, y_true, alea, epis, tota, subset, f"{subset}/{group}/{method}", i, suffix="pdf")
