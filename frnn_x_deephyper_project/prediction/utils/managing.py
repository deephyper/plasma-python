...
def average_prediction(y_pred_models, idx=None, weights=None, sample_weights=None):
    avg_y_pred = []
    if sample_weights is not None:
        for y_pred, weights in zip(y_pred_models, sample_weights):
            avg_y_pred.append(np.einsum('ij...,ij...->j...', weights, y_pred))
    elif idx is None:
